...
struct TreeNode {
    ...
    friend std::ostream &operator<<(std::ostream &os, TreeNode *rhs) {
        const int d = rhs->get_max_depth(rhs);
        ...
        const auto rows_disp = rhs->get_row_display(rhs);
        // then format these into a text representation...
        auto formatted_rows = rhs->row_formatter(rows_disp);
...
private:
    int get_max_depth(TreeNode *root) const {
        if (!root) return 1;
        return std::max(get_max_depth(root->left), get_max_depth(root->right)) + 1;
    }
    ...
    display_rows get_row_display(TreeNode *root) const {
